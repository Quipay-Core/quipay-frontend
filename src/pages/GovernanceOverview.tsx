...
async function contractRead<T>(functionName: string): Promise<T | null> {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const [a, t] = await Promise.all([
        contractRead<string>("admin"),
        contractRead<bigint>("EMERGENCY_MULTISIG_THRESHOLD"),
      ]);
      const signerReads = await Promise.all([
        contractRead<string>("emergencySigners"),
      ]);
      setAdmin(a);
      setSigners(signerReads.filter((s): s is string => !!s));
      setThreshold(t !== null ? Number(t) : null);
      setLoading(false);
    })();
  }, []);
        ...
        {isAdmin && (
          <div className="rounded-2xl border border-white/[0.07] bg-[#0a0a0a] p-6">
            ...
            <p className="mt-4 text-[12px] text-neutral-700">
              Emergency signers are set at deploy time. To rotate them, call{" "}
              <code className="font-mono text-neutral-500">
                rotateEmergencySigner
              </code>{" "}
